-- Update public holiday refresh functions after UUID people.id migration (009).
-- Replaces bigint person_id args with uuid.
...
BEGIN
  ...
  IF r = 'AU' THEN
    ...
    WHERE
      ...
      AND (d_start IS NULL OR c.holiday_date >= d_start)
      AND (d_end IS NULL OR c.holiday_date <= d_end);
  ELSE
    INSERT INTO person_public_holidays (person_id, holiday_date, name, holiday_type)
    SELECT
      p_person_id,
    ...
    WHERE
      c.year IN (y0, y1)
      AND (c.is_national = TRUE
        OR (c.region_codes IS NOT NULL AND r = ANY (c.region_codes)))
      AND (d_start IS NULL OR c.holiday_date >= d_start)
      AND (d_end IS NULL OR c.holiday_date <= d_end);
  END IF;
END;
